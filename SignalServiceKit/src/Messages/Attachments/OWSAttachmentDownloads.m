...
static const BOOL kAttachmentDownloadNoCdn = YES;
...
- (void)tryToStartNextDownload
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self retrieveAttachmentForJob:job
            ...
            noCdn:kAttachmentDownloadNoCdn];
    });
}
                           ...
                           noCdn:(BOOL)noCdn
{
    ...
    dispatch_async([OWSDispatch attachmentsQueue], ^{
        void (^successHandler)(NSString *encryptedDataFilePath) = ^(NSString *encryptedDataFilePath) {
            [self decryptAttachmentPath:encryptedDataFilePath
                      attachmentPointer:attachmentPointer
                                success:markAndHandleSuccess
                                failure:markAndHandleFailure];
        };
        void (^failureHandlerParam)(NSURLSessionTask *_Nullable task, NSError *error) = ^(NSURLSessionTask *_Nullable task, NSError *error) {
            if (attachmentPointer.serverId < 100) {
        ...
        if (noCdn) {
            [self getAttachmentLocationNoCdn:job
                attachmentPointer:(TSAttachmentPointer *)attachmentPointer
                success:successHandler
                failure:failureHandlerParam];
        } else {
...
- (void)getAttachmentLocationNoCdn:(OWSAttachmentDownloadJob *)job
                 attachmentPointer:(TSAttachmentPointer *)attachmentPointer
                           ...
                           failure:(void (^)(NSURLSessionTask *_Nullable task, NSError *error))failureHandlerParam
{
    ...
    TSRequest *formRequest = [OWSRequestFactory attachmentRequestWithAttachmentId:attachmentPointer.serverId];
    ...
    [self.networkManager makeRequest:formRequest
        success:^(NSURLSessionDataTask *task, id _Nullable formResponseObject) {
            if (![formResponseObject isKindOfClass:[NSDictionary class]]) {
                ...
                failureHandlerParam(task, [NSError errorWithDomain:@"" code:200 userInfo:@{@"Error reason": @"Invalid response"}]);
                return;
            }
            ...
            if (![location isKindOfClass:[NSString class]] || location.length < 1) {
                ...
                failureHandlerParam(task, [NSError errorWithDomain:@"" code:200 userInfo:@{@"Error reason": @"Invalid location"}]);
                return;
            }
        
            [self downloadJob:job attachmentPointer:attachmentPointer success:successHandler failure:failureHandlerParam location:location];
        }
        failure:^(NSURLSessionDataTask *task, NSError *error) {
            OWSLogError(@"Failed to get attachment form: %@", error);
             ...
             location:(nullable NSString *)location
{
    ...
    if (location.length > 0) {
        ...
        manager = self.cdnSessionManager;
        NSString *urlPath = [NSString stringWithFormat:@"attachments/%llu", attachmentPointer.serverId];
        url = [[NSURL alloc] initWithString:urlPath relativeToURL:manager.baseURL];
    }
