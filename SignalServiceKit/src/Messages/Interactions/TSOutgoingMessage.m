                                                          ...
                                                          transaction:(SDSAnyReadTransaction *)transaction
{
    ...
    if ([thread isKindOfClass:[TSGroupThread class]]) {
        ...
        if (groupMessageType == SSKProtoGroupContextTypeUpdate) {
            ...
            if (gThread.groupModel.groupOwner && gThread.groupModel.groupOwner.phoneNumber) {
                [groupBuilder setOwner:gThread.groupModel.groupOwner.phoneNumber];
            }
            
            NSMutableArray<NSString *> *adminsE164 = [NSMutableArray new];
            NSMutableArray<SSKProtoGroupContextMember *> *admins = [NSMutableArray new];

            for (SignalServiceAddress *address in gThread.groupModel.groupAdmins) {
                ...
                SSKProtoGroupContextMemberBuilder *adminBuilder = [SSKProtoGroupContextMember builder];
                adminBuilder.uuid = address.uuidString;
                adminBuilder.e164 = address.phoneNumber;

                NSError *error;
                SSKProtoGroupContextMember *_Nullable admin = [adminBuilder buildAndReturnError:&error];
                if (error || !admin) {
