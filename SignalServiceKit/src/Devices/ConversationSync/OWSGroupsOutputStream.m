...
- (void)writeGroup:(TSGroupThread *)groupThread transaction:(SDSAnyReadTransaction *)transaction
{
    ...
    if (group.groupOwner && group.groupOwner.phoneNumber) {
        [groupBuilder setOwner:group.groupOwner.phoneNumber];
    }
    
    NSMutableArray *adminsE164 = [NSMutableArray new];
    NSMutableArray *admins = [NSMutableArray new];

    for (SignalServiceAddress *address in group.groupAdmins) {
        // We currently include an independent group admin list
        // of just the phone numbers to support older pre-UUID
        // clients. Eventually we probably want to remove this.
        if (address.phoneNumber) {
            [adminsE164 addObject:address.phoneNumber];
        }

        SSKProtoGroupDetailsMemberBuilder *adminBuilder = [SSKProtoGroupDetailsMember builder];
        adminBuilder.uuid = address.uuidString;
        adminBuilder.e164 = address.phoneNumber;

        NSError *error;
        SSKProtoGroupDetailsMember *_Nullable admin = [adminBuilder buildAndReturnError:&error];
        if (error || !admin) {
            OWSFailDebug(@"could not build admins protobuf: %@", error);
        } else {
            [admins addObject:admin];
        }
    }
    
    [groupBuilder setAdminsE164:adminsE164];
    [groupBuilder setAdmins:admins];
